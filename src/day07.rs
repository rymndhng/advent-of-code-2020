...
pub fn main() -> std::io::Result<()>  {
    ...
    let entries: HashMap<String, Vec<BagSpec>> = reader.lines()
...
pub fn part_1(key: &str, entries: &HashMap<String,Vec<BagSpec>>) -> usize {
    ...
    for (bag_name, contains) in entries.iter() {
        for bag_spec in contains {
            let set = reverse_lookup.entry(&bag_spec.name).or_insert(HashSet::new());
...
pub fn parse_line(s: String) -> (String, Vec<BagSpec>) {
    ...
    loop {
        match words.next() {
            Some("contain") => { bag_name = bag_name.trim().to_string(); break},
            Some(s) => { bag_name.push_str(s); bag_name.push(' ')},
    ...
    for word in words {
        match word {
            ...
            s => {
                match s.parse::<i32>() {
                    Ok(c) => { count = c },
                    Err(_) => { name.push_str(s); name.push(' ')}
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn day7_parse_group() {
        let bag1 = BagSpec { name: "bright white bags".to_string(), count: 1};
        let bag2 = BagSpec { name: "muted yellow bags".to_string(), count: 2};


        let expected = ("light red bags".to_string(), vec!(bag1, bag2));

        assert_eq!(expected,
            parse_line("light red bags contain 1 bright white bag, 2 muted yellow bags.".to_string())
        );
    }
}
